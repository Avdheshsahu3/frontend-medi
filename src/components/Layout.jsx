...
function PharmacyLogo({ className = '' }) {
  return (
    <span className={`inline-flex items-center justify-center rounded-md bg-[#2563C9] text-white shadow-sm ${className}`} aria-hidden="true">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" strokeWidth="3" strokeLinecap="round">
...
const links = [
  { to: '/dashboard', label: 'Dashboard', icon: 'dashboard', roles: ['ADMIN', 'PHARMACIST', 'STAFF'] },
  { to: '/inventory', label: 'Inventory', icon: 'inventory', roles: ['ADMIN', 'PHARMACIST', 'STAFF'] },
  { to: '/sell', label: 'New Sale', icon: 'sale', roles: ['ADMIN', 'PHARMACIST', 'STAFF'] },
  { to: '/suppliers', label: 'Suppliers', icon: 'suppliers', roles: ['ADMIN', 'PHARMACIST', 'STAFF'] },
  ...
  { to: '/settings', label: 'Settings', icon: 'profile', roles: ['ADMIN', 'PHARMACIST', 'STAFF'] },
]
  ...
  return (
    <div className="flex min-h-screen bg-white">
      <aside className="fixed top-0 left-0 bottom-0 z-50 w-64 border-r border-white/10 p-5 backdrop-blur overflow-auto sidebar-surface">
        <div className="mb-6 rounded-2xl border border-[#2563C9] bg-[#2563C9] p-3">
        ...
        <nav className="space-y-1">
          {links.filter((l) => l.roles.includes(user?.role)).map((l) => (
            <NavLink
              ...
              to={l.to}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-xl px-3 py-2 text-sm transition ${isActive ? 'bg-[#2563C9] text-white shadow-lg shadow-blue-900/30' : 'text-white hover:bg-[#2563C9] hover:text-white'}`
              }
            >
              <span className="flex h-4 w-4 items-center justify-center text-white [&>svg]:text-white">
                <NavIcon type={l.icon} />
              </span>
              <span>{l.label}</span>
            </NavLink>
      ...
      <div className="flex-1 ml-64">
        <header className="fixed top-0 left-64 right-0 z-40 flex items-center justify-between border-b border-white/10 px-6 py-3 backdrop-blur navbar-surface">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="rounded-full border border-[#2563C9] bg-[#2563C9] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-white">{user?.role}</span>
            <button className="rounded-lg border border-[#2563C9] bg-[#2563C9] px-3 py-2 text-sm text-white transition hover:border-[#1D4FA3] hover:bg-[#1D4FA3] hover:text-white" onClick={() => { logout(); navigate('/login') }}>Logout</button>
          </div>
        </header>
